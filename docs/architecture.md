...
## 8. Despliegue con Docker (Opción B — un solo origen)

`docker-compose.yml` en la raíz levanta los 4 servicios: `postgres`, `rabbitmq` (presente por la narrativa del PDR pero **aún sin usar** — el chat corre sobre broker STOMP en memoria y los eventos son en proceso), `backend` (imagen propia, `backend/Dockerfile` multi-stage Maven→JRE 21) y `frontend` (`nginx:alpine` que sirve el build de Vite **y** hace de reverse-proxy al backend, `frontend/Dockerfile` + `frontend/nginx.conf`).

**Por qué reverse-proxy y no dos orígenes:** el navegador entra por una única dirección (`PUBLIC_BASE_URL`), así el front y el back son *same-origin* — CORS deja de intervenir y el upgrade WebSocket del chat pasa por la misma ruta (`location /ws` con cabeceras `Upgrade`/`Connection`). La alternativa (frontend llamando directo a `backend:8080`) obliga a mantener sincronizados el `allowed-origin` del CORS **y** el `setAllowedOrigins` del WebSocket con el origen real del front — más superficie de error para el mismo resultado.

**Regla de configuración:** el código no lleva URLs de infraestructura hardcodeadas. `application.yml` expone `spring.datasource.*` y `app.cors.allowed-origin` como `${VAR:<default local>}` (los defaults mantienen `mvn spring-boot:run` funcionando sin Docker); el contenedor `backend` los sobreescribe por env (`SPRING_DATASOURCE_URL` con host `postgres`, `APP_CORS_ALLOWED_ORIGIN`/`TRANSACTIONS_FRONTEND_URL` = `PUBLIC_BASE_URL`). El frontend hornea `VITE_API_BASE_URL=""` en build (llamadas relativas); `wsUrl()` deriva `ws(s)://` de `window.location` en ese caso. Secretos (`JWT_SECRET`, claves Stripe) por `.env` en la raíz — nunca en la imagen (`backend/config/` está en `.dockerignore`). Detalle operativo y gotchas en `docs/claude/estado-actual.md` §Despliegue con Docker.
